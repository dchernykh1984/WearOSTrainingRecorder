...
# Health Services speaks protobuf - and it is the only thing here that speaks the
# unshaded kind. The Data Layer's copy is shaded inside Play Services and comes
# with its own consumer rules, which is why the phone needs no rule at all.
#
# protobuf-javalite builds a message schema at runtime by reflecting on the
# generated fields *by name* - the names are embedded in a string constant the
# generated class carries. R8
